/**
 * dsh-hyperframes —— HyperFrames by HeyGen 视频创作技能插件：加载即把技能注册进 ctx.skills。
 *
 * 技能正文与资源随包分发（skills/ 目录），resourceBase 指向打包目录，
 * 技能正文里的相对引用（rules/、references/ 等）由技能加载器解析。
 *
 * @module dsh-hyperframes
 */
...
/** 随包分发的技能清单。 */
export const SKILL_NAMES = ["hyperframes", "hyperframes-cli", "hyperframes-registry", "website-to-hyperframes", "gsap"];
/** 打包技能目录的绝对路径。 */
    ...
    for (const skillName of SKILL_NAMES) {
        try {
            ...
            if (parsed.name === '' || parsed.description === '' || parsed.content === '') {
                console.warn('[dsh-hyperframes] 技能 ' + skillName + ' 的 frontmatter 不完整，已跳过。');
                continue;
            }
            ...
            console.warn('[dsh-hyperframes] 技能 ' + skillName + ' 加载失败：' + (error instanceof Error ? error.message : String(error)));
        }
    }
    if (typeof ctx.tools?.register === 'function') {
        disposers.push(ctx.tools.register({
            name: 'hyperframes_health',
            description: 'dsh-hyperframes 自检：检查随包技能资源（SKILL.md）是否完整。遇到问题时先运行本工具定位。',
            ...
            output: {
                ...
                render(_args, value) {
                    ...
                    const lines = ['dsh-hyperframes 自检' + (rec.ok === true ? '：技能资源完整。' : '：发现缺失。')];
            ...
            async execute() {
                ...
                return { ok: skills.every((s) => s.ok), plugin: 'dsh-hyperframes', skills };
            },
        }));
    }
