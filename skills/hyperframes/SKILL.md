---
name: hyperframes
description: "HyperFrames by HeyGen: HTML is the source of truth for video. Author video compositions with HTML + CSS + GSAP (data-* timing attributes, GSAP timelines, CSS appearance). Use when asked to build any HTML-based video content, add captions or subtitles synced to audio, generate text-to-speech narration, create audio-reactive animation (beat sync, glow, pulse driven by music), add animated text highlighting (marker sweeps, hand-drawn circles, burst lines, scribble, sketchout), or add transitions between scenes (crossfades, wipes, reveals, shader transitions). For CLI commands see the hyperframes-cli skill; registry blocks see hyperframes-registry; website-to-video see website-to-hyperframes; GSAP API reference see gsap. 中文：用 HyperFrames（HTML 写视频）制作视频、字幕、配音、音频响应动画、转场时使用。"
compatibility: Requires Node.js >= 22 and FFmpeg on PATH; rendering runs through the `npx hyperframes` CLI.
allowed-tools: Bash
---
